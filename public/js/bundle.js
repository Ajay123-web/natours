...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYWpheS0xNCIsImEiOiJja3lvYzQwZzgzeHhjMm9wODB4bGdsMDJvIn0.2qoGBdHgsqbOrQxO-RMXxQ';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/ajay-14/ckyolx736a3s414qpe1ipau0x',
    scrollZoom: false // center: [lng, lat],
    // zoom: 10,
    // interactive: false

  }); //mapboxgl is a map object that is created by including the script in tour.pug

  var bounds = new mapboxgl.LngLatBounds(); //represents a rectangular geographical area on a map
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>\n        Day ".concat(loc.day, ": ").concat(loc.description, "\n      </p>")).addTo(map); //INCLUDE CURRENT LOCATION IN BOUNDS
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(tourId) {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            stripe = Stripe('pk_test_51KMcVRSGy9VXmPmnGUMhxGfvc9DJM9Ob37xZ4rQ1VFD4I5dK5fGPj2bAsWx2Zztl6H9udKNfLUuHQmwMVwaDlhof00PFtMqRjE');
            _context.prev = 1;
            _context.next = 4;
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault();
    var form = new FormData(); //store data as a key-value pair as done in a form
